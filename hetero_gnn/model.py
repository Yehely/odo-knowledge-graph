...
from hetero_gnn.config import (
    ...
    HEAD_DROPOUT, HEAD_IN_DIM, HIDDEN, MODEL_SYSTEM_EMB_DIM, POOLING_MODE,
    TARGET_EMB_DIM, TARGET_HIDDEN_IN,
)
...
class HeteroOpioidGNN(nn.Module):
    ...
    pooling_mode            : "attention" (default) or "mean" — see config.POOLING_MODE
    ...
    def __init__(
        ...
        super().__init__()
        self.hidden = hidden
        assert pooling_mode in ("attention", "mean"), pooling_mode
        ...
        if self.pooling_mode == "attention":
            # Attention layers for scoring neighbors (one small scoring head per hop)
